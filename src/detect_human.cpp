...
Cloud crop3DObjectFromPointCloud(const deep_object_detection::Object& object, Cloud objectcloud, int image_cols,tf::Vector3 robotPosition,tf::StampedTransform transform )
{
	Cloud objectpc;
  ...
  //save in txt
  std::ofstream outfile("/home/lin/catkin_ws/src/detect_human/result/resultdec.txt", std::ios_base::app);
  if(!outfile)
  {
  	std::cout<<"error";
  }
    ...
    for(int i = 0; i < srv.response.objects.size() ; i++)
    {
      ...
      if(srv.response.objects[i].label == "person")
      {
        if(pcl_depth_reg_data.points.size() > 0)
        	crop3DObjectFromPointCloud(srv.response.objects[i], pcl_depth_reg_data, cv_ptr->image.cols, robot_position, transform);
      }
    }
  }
